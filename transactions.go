...
func (t *Transaction) Execdata() ([]byte, error) {
	...
	if t.Signature != nil {
		return contracts.V2.WalletMainModule.Encode("execute", encodedTxns, t.Nonce, t.Signature)
	} else {
		return contracts.V2.WalletMainModule.Encode("selfExecute", encodedTxns)
	}
}
...
func GetWalletNonce(provider *ethrpc.Provider, walletConfig core.WalletConfig, walletContext WalletContext, space *big.Int, blockNum *big.Int) (*big.Int, error) {
	...
	contract := ethcontract.NewContractCaller(walletAddress, contracts.V2.WalletMainModule.ABI, provider)
...
func DecodeExecdata(data []byte) (Transactions, *big.Int, []byte, error) {
	...
	executeMethod := contracts.V2.WalletMainModule.ABI.Methods["execute"]
	selfExecuteMethod := contracts.V2.WalletMainModule.ABI.Methods["selfExecute"]
