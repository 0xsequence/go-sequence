...
func TestTransactionVerbose(t *testing.T) {
	t.Run("v1", func(t *testing.T) {
		...
		//txSubDigest, err := v1v2.SubDigest(wallet.GetChainID(), wallet.Address(), signedTx.Digest)
		//assert.NoError(t, err)

		walletConfig, weight, err := v1v2.GenericRecoverWalletConfigFromDigest[*v1.WalletConfig](signedTx.Digest.Bytes(), signedTx.Signature, wallet.Address(), testutil.V1SequenceContext(), testChain.ChainID(), testChain.Provider)
		assert.NoError(t, err)
		assert.GreaterOrEqual(t, weight.Cmp(big.NewInt(int64(walletConfig.Threshold()))), 0)

		walletAddress, err := v1v2.AddressFromWalletConfig(walletConfig, testutil.V1SequenceContext())
		assert.NoError(t, err)

		assert.Equal(t, wallet.Address(), walletAddress)

		expectedMetaTxnID, _, err := v1v2.ComputeMetaTxnIDFromDigest(testChain.ChainID(), walletAddress, signedTx.Digest)
		assert.NoError(t, err)
	...
	t.Run("v2", func(t *testing.T) {
		...
		walletConfig, weight, err := v1v2.GenericRecoverWalletConfigFromDigest[*v2.WalletConfig](signedTx.Digest.Bytes(), signedTx.Signature, wallet.Address(), testutil.V1SequenceContext(), testChain.ChainID(), testChain.Provider)
		assert.NoError(t, err)
		assert.GreaterOrEqual(t, weight.Cmp(big.NewInt(int64(walletConfig.Threshold()))), 0)

		walletAddress, err := v1v2.AddressFromWalletConfig(walletConfig, testutil.V2SequenceContext())
		assert.NoError(t, err)
	...
	// 	walletConfig, weight, err := v1v2.GenericRecoverWalletConfigFromDigest[*v3.WalletConfig](signedTx.Digest.Bytes(), signedTx.Signature, wallet.Address(), testutil.V3SequenceContext(), testChain.ChainID(), testChain.Provider)
	// 	assert.NoError(t, err)
	// 	assert.GreaterOrEqual(t, weight.Cmp(big.NewInt(int64(walletConfig.Threshold()))), 0)

	// 	walletAddress, err := v1v2.AddressFromWalletConfig(walletConfig, testutil.V3SequenceContext())
	// 	assert.NoError(t, err)

	// 	assert.Equal(t, wallet.Address(), walletAddress)

	// 	expectedMetaTxnID := v1v2.MetaTxnID(hex.EncodeToString(signedTx.Digest.Bytes()))
	// 	assert.NoError(t, err)
...
func TestTransactionToGuestModuleBasic(t *testing.T) {
	t.Run("v1", func(t *testing.T) {
		...
		metaTxnID, _, err := v1v2.ComputeMetaTxnID(
			testChain.ChainID(),
			testChain.V1SequenceContext().GuestModuleAddress,
			bundle, nil, v1v2.MetaTxnGuestExec,
		)
		assert.NoError(t, err)
		...
		// NOTE: we cannot use v1v2.WaitForMetaTxn and listen for the metaTxnID, as there is no NonceChange
	...
	// 	payload, err := v1v2.ConvertTransactionsToV3Payload(bundle, big.NewInt(0), big.NewInt(0))
	// 	assert.NoError(t, err)
	...
	// 	metaTxnID := v1v2.MetaTxnID(metaTxnHash.Hex()[2:])
...
func TestTransactionToGuestModuleDeployAndCall(t *testing.T) {
	t.Run("v1", func(t *testing.T) {
		...
		_, walletFactoryAddress, walletDeployData, err := v1v2.EncodeWalletDeployment(wallet.GetWalletConfig(), wallet.GetWalletContext())
		assert.NoError(t, err)
		...
		walletBundle := v1v2.Transactions{
			{
				To: wallet.Address(),
				Transactions: v1v2.Transactions{
					{
		...
		metaTxnID, _, err := v1v2.ComputeMetaTxnID(
			testChain.ChainID(),
			testChain.V1SequenceContext().GuestModuleAddress,
			guestBundle, nil, v1v2.MetaTxnGuestExec,
		)
		assert.NoError(t, err)
		...
		// spew.Dump(v1v2.DecodeRevertReason(receipt.Logs))
		...
		// Assert v1v2.WaitForMetaTxn is able to find the metaTxnID
		result, _, _, err := v1v2.FetchMetaTransactionReceipt(context.Background(), testChain.ReceiptsListener, metaTxnID)
		// metaStatus, _, err := v1v2.WaitForMetaTxn(context.Background(), testChain.Provider, metaTxnID)
		assert.NoError(t, err)
		assert.True(t, result.Status == v1v2.MetaTxnExecuted)
	...
	// 	payload, err := v1v2.ConvertTransactionsToV3Payload(walletBundle, signedWalletBundle.Space, signedWalletBundle.Nonce)
	// 	assert.NoError(t, err)
...
func TestTransactionSignatureReduction(t *testing.T) {
	...
	data2, err := v1v2.ReduceExecdataSignatures(big.NewInt(137), data)
	assert.NoError(t, err)
...
func TestRawTransactionsEncoding(t *testing.T) {
	...
	transactions, err := v1v2.DecodeRawTransactions(data)
	assert.NoError(t, err)
