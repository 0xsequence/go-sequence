...
type WalletConfig interface {
	...
	SignersWeight(signers []Signer) uint16
...
type Signer struct {
	...
	IsSapient   bool
	SapientHash common.Hash
}

func SapientSigner(address common.Address, sapientHash common.Hash) Signer {
	return Signer{Address: address, IsSapient: true, SapientHash: sapientHash}
}
