// sequence-indexer v0.4.0 7b021ac5e55fb2e3ebcbbd0631ed4f7d27667a48
// --
...
func WebRPCSchemaHash() string {
	return "7b021ac5e55fb2e3ebcbbd0631ed4f7d27667a48"
}
...
type MarketplaceTopOrdersFilter struct {
	...
	ExcludeUser       *string   `json:"excludeUser" cbor:"5"`
}
...
type OrderbookOrderFilter struct {
	IsListing            *bool    `json:"isListing" cbor:"1"`
	UserAddresses        []string `json:"userAddresses" cbor:"2"`
	TokenIDs             []string `json:"tokenIds" cbor:"3"`
	ExcludeUserAddresses []string `json:"excludeUserAddresses" cbor:"4"`
	AfterBlockNumber     uint64   `json:"afterBlockNumber" cbor:"5"`
	AfterCreatedAt       int64    `json:"afterCreatedAt" cbor:"6"`
	BeforeExpiry         int64    `json:"beforeExpiry" cbor:"7"`
	UserAddress          *string  `json:"userAddress" cbor:"8"`
	ExcludeUserAddress   *string  `json:"excludeUserAddress" cbor:"9"`
}
...
type Indexer interface {
	...
	// GetMarketplaceOrders retrieves marketplace orders with comprehensive filtering and pagination support.
	//
	// This method provides a flexible way to query marketplace orders with various filtering criteria,
	// supporting both buy orders (offers) and sell orders (listings) across different collections and currencies.
	//
	// Parameters:
	// - page: Optional pagination control (Page object with limit and after cursor for result navigation)
	// - marketplaceContractAddress: The marketplace contract address to query orders from (required)
	// - collectionAddress: The NFT collection contract address to filter orders for (required)
	// - filter: MarketplaceOrderFilter object with comprehensive filtering options:
	// • isListing: Optional boolean to filter by order type (true=listings/sell orders, false=offers/buy orders, omit=both)
	// • userAddresses: Optional array of user addresses to include in results
	// • currencyAddresses: Array of currency contract addresses to filter by (empty=all currencies accepted)
	// • tokenIds: Array of specific token IDs to filter orders for (empty=all tokens in collection)
	// • excludeUserAddresses: Optional array of user addresses to exclude from results
	// • afterBlockNumber: Filter orders created after this block number (0=no filter)
	// • afterCreatedAt: Filter orders created after this Unix timestamp (0=no filter)
	// • beforeExpiry: Filter out orders that expire before this Unix timestamp (0=no filter, includes non-expiring orders)
	// • orderStatuses: Array of OrderStatus values to include (OPEN, CLOSED, CANCELLED)
	//
	// Returns:
	// - page: Updated pagination information with cursors for next/previous page navigation
	// - orders: Array of MarketplaceOrder objects matching all specified criteria, ordered by creation time
	//
	// Example Usage:
	// - Get all active listings for a collection: filter.isListing=true, filter.orderStatuses=[OPEN]
	// - Get offers from specific users: filter.isListing=false, filter.userAddresses=[user1, user2]
	// - Get orders for specific tokens: filter.tokenIds=[1, 2, 3]
	...
	// GetMarketplaceTopOrders retrieves the most competitive orders for specific tokens.
	//
	// This method is optimized for price discovery, returning the best available orders for each requested token.
	// Perfect for displaying current market prices and finding the most attractive trading opportunities.
	//
	// Parameters:
	// - marketplaceContractAddress: The marketplace contract address to query orders from (required)
	// - collectionAddress: The NFT collection contract address containing the target tokens (required)
	// - filter: MarketplaceTopOrdersFilter object specifying order selection criteria:
	// • currencyAddresses: Array of currency contract addresses to consider (empty=all currencies)
	// • tokenIds: Array of specific token IDs to find top orders for (required, non-empty)
	// • isListing: Order type to retrieve (true=listings/sell orders, false=offers/buy orders)
	// • priceSort: Price sorting preference (ASC=lowest prices first, DESC=highest prices first)
	// • excludeUser: Optional user address to exclude from results (useful for hiding own orders)
	//
	// Returns:
	// - orders: Array of MarketplaceOrder objects representing the top-priced orders for each token.
	// Results are filtered to active orders only and sorted according to priceSort preference.
	// For listings (sell orders): ASC returns cheapest listings, DESC returns most expensive
	// For offers (buy orders): ASC returns lowest offers, DESC returns highest offers
	//
	// Example Usage:
	// - Find cheapest listings: filter.isListing=true, filter.priceSort=ASC
	// - Find highest offers: filter.isListing=false, filter.priceSort=DESC
	// - Market overview excluding own orders: filter.excludeUser=myAddress
