...
type TestChain struct {
	...
	Relayer *relayer.Client // helper to track RpcRelayer client
...
func (c *TestChain) SetRelayer(relayerURL string) error {
	rpcRelayer, err := relayer.NewClient(relayerURL, "", c.Provider, relayer.Options{ReceiptsListener: c.ReceiptsListener})
	if err != nil {
		return err
	}
...
func (c *TestChain) V3DummySequenceWalletWithIntentConfig(seed uint64, calls []*v3.CallsPayload, optSkipDeploy ...bool) (*sequence.Wallet[core.WalletConfig], error) {
	...
	intentConfig, err := sequence.CreateIntentConfiguration(owner.Address(), calls, 0, nil, nil)
	if err != nil {
		return nil, err
	}
