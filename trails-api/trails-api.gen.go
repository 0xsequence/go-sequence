// trails-api v1-25.11.20+c2a87fea 3a288cdd32693d6fa78e00f46de090086b9c30ec
// --
...
func WebRPCSchemaVersion() string {
	return "v1-25.11.20+c2a87fea"
}

// Schema hash generated from your RIDL schema
func WebRPCSchemaHash() string {
	return "3a288cdd32693d6fa78e00f46de090086b9c30ec"
}
...
type TrailsClient interface {
	...
	// GetExactOutputRoutes will return a list of tokens, when given a destination chain and token,
	// that can be used to pay/send from an origin chain the exact output amount on the
	// destination chain.
	//
	// The request will include the destination chain and token desired, along with the
	// owner address of who will be sending the funds. This allows us to filter the list
	// of possible origin tokens to only those the owner has a balance on.
	//
	// The response is going to be a list of origin tokens and their chains which can be used
	// to fullfill the exact output request.
	//
	// aka, the 'pay' routes
	...
	// GetExactInputRoutes will return a list of tokens, when given an origin chain and token,
	...
	// user can specify a destination chain and token to further filter the results, and
	// see if any token routes are availabled / returned.
	//
	// The resposne is going to be a list of destination tokens and their chains which can be
	// reached from the origin token and chain.
	//
	// aka, the 'swap' routes
	...
	// GetPopularTokens will return a list of popular tokens based on the provided filters.
	// This is useful for populating token selection lists.
	GetPopularTokens(ctx context.Context, getPopularTokensRequest GetPopularTokensRequest) (*GetPopularTokensResponse, error)
...
type GetPopularTokensRequest struct {
	// all of these are optional.....
	// TODO: still thinking about filters
	RouteProvider *string  `json:"routeProvider"`
	Limit         *uint32  `json:"limit"`
	ChainIds      []uint64 `json:"chainIds"`
}

type GetPopularTokensResponse struct {
	Tokens []*TokenInfo `json:"tokens"`
}
...
func (c *trailsClient) GetPopularTokens(ctx context.Context, getPopularTokensRequest GetPopularTokensRequest) (*GetPopularTokensResponse, error) {
	out := struct {
		Ret0 *GetPopularTokensResponse
	}{}

	resp, err := doHTTPRequest(ctx, c.client, c.urls[15], getPopularTokensRequest, &out.Ret0)
	if resp != nil {
		cerr := resp.Body.Close()
		if err == nil && cerr != nil {
			err = ErrWebrpcRequestFailed.WithCausef("failed to close response body: %w", cerr)
...
func (c *trailsClient) SearchTokens(ctx context.Context, searchTokensRequest SearchTokensRequest) (*SearchTokensResponse, error) {
	...
	resp, err := doHTTPRequest(ctx, c.client, c.urls[16], searchTokensRequest, &out.Ret0)
	if resp != nil {
		cerr := resp.Body.Close()
		if err == nil && cerr != nil {
			err = ErrWebrpcRequestFailed.WithCausef("failed to close response body: %w", cerr)
...
const WebrpcHeaderValue = "webrpc@v0.31.2;gen-golang@v0.23.3;trails-api@v1-25.11.20+c2a87fea"
